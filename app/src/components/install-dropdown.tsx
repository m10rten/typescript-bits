...
function scopedEntries(module: string): Entry[] {
  const scoped = `@typescript-bits/${module}`;
  return [
    { id: "pnpm", label: "pnpm", command: `pnpm add ${scoped}` },
    { id: "bun", label: "bun", command: `bun add ${scoped}` },
    { id: "npm", label: "npm", command: `npm install ${scoped}` },
    { id: "yarn", label: "yarn", command: `yarn add ${scoped}` },
    { id: "deno", label: "deno", command: `deno add npm:${scoped}` },
  ];
}
...
export function InstallDropdown({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Scoped package install (module only) */}
      {showInstall && scopedModule && (
        <div className="rounded-lg border bg-zinc-950 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-2">
            <div className="flex items-center gap-2">
              <span className="text-xs text-zinc-400 font-mono">install (module)</span>
              <select
                value={scopedPm}
                onChange={(e) => setScopedPm(e.target.value)}
                className="bg-zinc-900 text-xs text-zinc-300 border border-zinc-700 rounded px-2 py-0.5 font-mono focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
                aria-label="Package manager">
                {scopedOptions.map((e) => (
                  <option key={e.id} value={e.id}>
            ...
            <CodeCopyButton source={selectedScoped.command} label={selectedScoped.label} />
          </div>
          <div
            className="px-4 py-3 font-mono text-sm text-zinc-200 select-all"
            style={{ fontVariantLigatures: "none" }}>
            {selectedScoped.command}
          </div>
        </div>
      )}
